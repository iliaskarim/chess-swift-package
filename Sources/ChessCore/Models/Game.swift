...
public class Game {
  ...
  public func move(_ notation: String) throws {
...
extension Game: BoardDataSource {
  var enPassant: Square? {
    ...
    return Square.File.allCases.map { file in
      .init(file: file, rank: Square.Rank(rawValue: pawn.startRank.rawValue + 2 * pawn.forwardUnitVector.ranks)!)
    }.first { square in
      !moves.isEmpty && moves.last!.description.hasPrefix(square.description) && !moves.contains { move in
        let intermediateRank = Square.Rank(rawValue: pawn.startRank.rawValue + pawn.forwardUnitVector.ranks)!
        let intermediateSquare = Square(file: square.file, rank: intermediateRank)
        return move.description.hasPrefix(intermediateSquare.description)
      }
    }
  }
